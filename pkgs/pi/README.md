...
cp "$tmpdir/package/package-lock.json" ./pkgs/pi/package-lock.json
```
...
```bash
nix run nixpkgs#prefetch-npm-deps -- ./pkgs/pi/package-lock.json
```
...
- The package definition is in `pkgs/pi/default.nix`.
