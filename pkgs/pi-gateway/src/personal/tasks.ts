...
function ensureUniquePath(baseDir: string, slug: string): { path: string; id: string } {
  let candidate = slug;
  let index = 2;
...
function extractTaskBody(input: string): string | null {
  const patterns = [
    /^task\s*:\s*(.+)$/i,
    /^todo\s*:\s*(.+)$/i,
...
function maybeExtractDue(body: string): { action: string; due?: string } {
